      ...
      <draggable tag="ul" :list="selectedWidget.tabs"
        ...
        <li v-for="(tpItem, tpIdx) in selectedWidget.tabs" :key="tpIdx" class="col-item">
          ...
          <el-checkbox v-model="tpItem.options.active" disabled @change="(evt) => onTabPaneActiveChange(evt, tpItem)"
                       style="margin-right: 8px">激活</el-checkbox>
        ...
        <div>
          <el-button type="text" @click="addTabPane(selectedWidget)">增加选项卡页</el-button>
        </div>
      </draggable>
    </el-form-item>
  </div>
...
li.col-item span.col-span-title {
  display: inline-block;
  font-size: 13px;
  width: 120px;
}

li.col-item .col-delete-button {
  margin-left: 6px;
}
...
.panes-setting .drag-option {
  cursor: move;
}

.panes-setting li.ghost {
  background: #fff;
