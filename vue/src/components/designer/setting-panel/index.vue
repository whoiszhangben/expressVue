<template>
  <el-container class="panel-container">
              ...
              <el-collapse v-model="widgetActiveCollapseNames" class="setting-collapse">
                <el-collapse-item name="1" v-if="showCollapse(commonProps)" title="常见属性">
                  <template v-for="(editorName, propName) in commonProps">
                    <component v-if="hasPropEditor(propName, editorName)" :is="getPropEditor(propName, editorName)" :key="propName"
    ...
    <el-dialog title="组件事件处理" :visible.sync="showWidgetEventDialogFlag"
  ...
  export default {
    ...
    methods: {
      ...
      hasPropEditor(propName, editorName) {
        ...
        console.log('hasPropEditor:', this.selectedWidget, originalPropName);
      ...
      showCollapse(propsObj) {
        ...
        for (let propName in propsObj) {
          ...
          if (this.hasPropEditor(propName, propsObj[propName])) {
            console.log("internal", propName, propsObj);
            result = true
            break
          }
        }
        console.log("outter", result);
        return result
      },
...
.setting-scrollbar ::v-deep .el-scrollbar__wrap {
  overflow-x: hidden;
  /* IE浏览器隐藏水平滚动条箭头！！ */
}
