      ...
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <a :href="file.url" download="">
            <i class="el-icon-download file-action" title="下载"></i></a>
          <i class="el-icon-delete file-action" title="移除" v-if="!field.options.disabled"
  ...
  export default {
    ...
    methods: {
      handleFileExceed() {
        ...
        this.$message.warning(uploadLimit);
      },
...
.dynamicPseudoAfter ::v-deep .el-upload.el-upload--text .el-icon-plus:after {
  content: var(--select-file-action);
}

.hideUploadDiv ::v-deep div.el-upload--picture-card {
  /* 隐藏最后的图片上传按钮 */
  display: none;
}

.hideUploadDiv ::v-deep div.el-upload--text {
  /* 隐藏最后的文件上传按钮 */
  display: none;
}

.hideUploadDiv ::v-deep div.el-upload__tip {
  /* 隐藏最后的文件上传按钮 */
  display: none;
}
