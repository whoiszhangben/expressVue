  ...
  <td class="table-cell" :class="[selected ? 'selected' : '', customClass]"
    ...
    <div class="table-cell-action" v-if="designer.selectedId === widget.id && widget.type === 'table-cell'">
      ...
      <el-dropdown trigger="click" @command="handleTableCellCommand" size="small">
        <i class="el-icon-menu" title="单元格操作"></i>
...
.table-cell .draggable-div {
  position: relative;
  height: 100%;
}

.table-cell .form-widget-list {
  border: 1px dashed #336699;
  margin: 3px;
  height: 100%;
  /*position: absolute;*/
...
.table-cell .table-cell-action i {
  font-size: 14px;
  color: #fff;
  margin: 0 5px;
  cursor: pointer;
...
.table-cell .table-cell-handler i {
  font-size: 14px;
  font-style: normal;
  color: #fff;
  margin: 4px;
