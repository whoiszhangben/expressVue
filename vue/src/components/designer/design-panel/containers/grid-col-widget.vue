<template>
  <el-col v-if="widget.type === 'grid-col'" class="grid-cell" v-bind="layoutProps"
    ...
    <div class="grid-col-action" v-if="designer.selectedId === widget.id && widget.type === 'grid-col'">
      ...
      <i class="el-icon-copy-document" title="复制组件" @click.stop="cloneGridCol(widget)"></i>
      <i class="el-icon-delete" title="移除组件" @click.stop="removeWidget"></i>
    </div>
...
.grid-cell .form-widget-list {
  min-height: 28px;
}

.grid-cell .grid-col-action {
  position: absolute;
  bottom: 0;
  right: -2px;
  height: 28px;
